...
{
  static uint32_t spiCallCount = 0;
  static uint32_t lastSpiLogTime = 0;

  uint32_t size = (Xend - Xstart +1 ) * (Yend - Ystart + 1);

  // OPTIMIZATION: Use compiler intrinsic for byte swap (faster than manual bit ops)
  for (size_t i = 0; i < size; i++) {
    color[i] = __builtin_bswap16(color[i]);
  }
  ...
  // Log before potentially blocking SPI call
  spiCallCount++;
  uint32_t spiStartTime = micros();

  // CRITICAL: This call may block indefinitely if SPI/DMA transfer hangs
  esp_err_t ret = esp_lcd_panel_draw_bitmap(panel_handle, Xstart, Ystart, Xend, Yend, color);

  uint32_t spiDuration = micros() - spiStartTime;

  // Check for errors or extremely long transfers
  if (ret != ESP_OK) {
    printf("[SPI_ERROR] esp_lcd_panel_draw_bitmap failed with error: 0x%x\n", ret);
  }

  // Log periodically (every 5 seconds)
  uint32_t now = millis();
  if (now - lastSpiLogTime >= 5000) {
    printf("[SPI_STATS] %lu SPI transfers in last 5s\n", spiCallCount);
    spiCallCount = 0;
    lastSpiLogTime = now;
  }

  // Warn if this specific transfer took too long
  if (spiDuration > 50000) {  // 50ms is very slow
    printf("[SPI_WARNING] Slow SPI transfer: %lu us (%lu ms), area: %d,%d to %d,%d, pixels: %lu\n",
           spiDuration, spiDuration / 1000, Xstart, Ystart, Xend, Yend, (uint32_t)size);
  }
}
